...
StackType_t LEDToggle_Task_Stack[128];
StackType_t SerialSend_Task_Stack[128];

StaticTask_t LEDToggle_Task_tcb;
StaticTask_t SerialSend_Task_tcb;
...
void start_task(void *argument)
{
  ...
  LEDToggle_Task_Handler = xTaskCreateStatic((TaskFunction_t)LEDToggle_Task,
                                             ( char *const)"LEDToggle_Task",
                                             ( uint32_t)128,
                                             (void *)NULL,
                                             (UBaseType_t)osPriorityLow,
                                             (StackType_t *)LEDToggle_Task_Stack,
                                             (StaticTask_t *)&LEDToggle_Task_tcb);

  SerialSend_Task_Handler = xTaskCreateStatic((TaskFunction_t)SerialSend_Task,
                                            ( char *const)"SerialSend_Task",
                                            ( uint32_t)128,
                                            (void *)NULL,
                                            (UBaseType_t)osPriorityLow,
                                            (StackType_t *)SerialSend_Task_Stack,
                                            (StaticTask_t *)&SerialSend_Task_tcb);
